...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            var queryArgs = [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}, _this.options.algo || ''];
            if(_this.options.algo) {
                queryArgs.push(_this.options.algo || '')
            }
            _this.daemon.cmd('getblocktemplate', queryArgs, function(results){
    ...
    function GetBlockTemplate(callback){
        var queryArgs = [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}];
        if(_this.options.algo) {
            queryArgs.push(_this.options.algo || '');
        }
