...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            _this.daemon.cmd('getblocktemplate', [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}, _this.options.algo || ''], function(results){
    ...
    function SubmitBlock(blockHex, callback){
        ...
        if (options.hasSubmitMethod){
            ...
            if(options.algo) {
                rpcArgs = [{'mode': 'submit', 'data': blockHex}, options.algo];
            }
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.testnet = rpcResults.getblockchaininfo.testnet === 'test' ? true : false;
            options.protocolVersion = rpcResults.getnetworkinfo.protocolversion;

            options.initStats = {
                connections: rpcResults.getnetworkinfo.connections,
                difficulty: rpcResults.getblockchaininfo.difficulties[options.coin.algo] * algos[options.coin.algorithm].multiplier,
    ...
    function GetBlockTemplate(callback){
        _this.daemon.cmd('getblocktemplate',
            [{"capabilities": [ "coinbasetxn", "workid", "coinbase/append" ], "rules": [ "segwit" ]}, _this.options.algo || ''],
            function(result){
                if (result.error){
